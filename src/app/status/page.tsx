...
export default function PublicStatusPage() {
  ...
  const getSystemStatus = () => {
    ...
    if (hasDegraded) {
      return {
        ...
        color: "bg-yellow-500",
        bgClass: "bg-yellow-50 text-yellow-900 border border-yellow-200",
        icon: "⚠️",
      };
    }
  ...
  const getServiceStatusHelper = (status: string) => {
    if (status === "operational") {
      return { label: "Operational", color: "bg-green-500", icon: "✓", text: "text-green-700 bg-green-50 border-green-200" };
    }
    if (status === "degraded") {
      return { label: "Degraded", color: "bg-yellow-500", icon: "!", text: "text-yellow-700 bg-yellow-50 border-yellow-200" };
    }
    return { label: "Down", color: "bg-red-500", icon: "✕", text: "text-red-700 bg-red-50 border-red-200" };
  };
    ...
    <div className="min-h-screen bg-[#f9fafb] text-gray-900 flex flex-col items-center">
      ...
      <div className="w-full max-w-[1280px] px-6 py-8 space-y-8 flex-1 flex flex-col">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
                <div className="pb-2 border-b border-gray-100">
                  ...
                  <p className="text-xs text-gray-500">Current status of monitored components</p>
                </div>

                {services.length === 0 ? (
                  <div className="p-12 text-center text-sm text-gray-500 border border-dashed border-gray-200 rounded-lg">
            ...
            <div className="space-y-6">
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
                <div className="pb-2 border-b border-gray-100 flex items-center justify-between">
                  <h3 className="text-lg font-bold text-gray-900">Ongoing Incidents</h3>
                ...
                {incidents.length === 0 ? (
                  ...
                  <div className="space-y-4 max-h-[500px] overflow-y-auto pr-1">
                    {incidents.map((incident) => (
                      <div key={incident.id} className="p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-4">
                        <div className="flex items-start justify-between border-b border-gray-200/60 pb-2">
                          <div className="space-y-0.5">
                            ...
                            <span className="inline-flex rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold text-red-800 uppercase tracking-wider">
                              {incident.severity}
                            </span>
                          </div>
                          <span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-bold text-gray-500 border border-gray-200 capitalize">
                            {incident.status}
                          </span>
                        </div>
        ...
        <footer className="pt-8 border-t border-gray-200 text-center text-xs text-gray-400">
          Powered by{" "}
          <Link href="/" className="font-semibold text-gray-500 hover:text-gray-900 underline">
            StatusForge
          </Link>
        </footer>
      </div>
