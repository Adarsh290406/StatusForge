...
export default function PublicStatusPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 p-6 flex flex-col items-center">
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200/50 pb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold tracking-tight text-zinc-900">StatusForge</span>
          </div>
          <Link href="/login" className="text-xs font-medium text-zinc-500 hover:text-zinc-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 rounded p-1" aria-label="Navigate to Admin Login">
            Admin Portal →
          </Link>
        </div>
        ...
        {loading && (
          <div className="space-y-4">
            <div className="h-24 w-full animate-pulse rounded-xl bg-zinc-200/60" />
            <div className="h-60 w-full animate-pulse rounded-xl bg-zinc-200/60" />
          </div>
        )}
        ...
        {!loading && (
          <>
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm space-y-4">
              <div>
                <h3 className="text-sm font-bold text-zinc-900">System Components</h3>
                <p className="text-xs text-zinc-500">Real-time operational status of core components</p>
              </div>

              {services.length === 0 ? (
                <div className="p-8 text-center text-sm text-zinc-500 border border-dashed border-zinc-200 rounded-lg">
                ...
                <div className="grid gap-3">
                  {services.map((service) => {
                    ...
                    return (
                      <div key={service.id} className="flex items-center justify-between p-3.5 rounded-lg border border-zinc-100 hover:border-zinc-200 transition-colors bg-zinc-50/30">
                        <div className="space-y-0.5">
                          <h4 className="text-sm font-bold text-zinc-800">{service.name}</h4>
                          {service.description && (
                            <p className="text-xs text-zinc-400 font-medium">{service.description}</p>
                          )}
                        </div>
                        <div className={`flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-bold border border-zinc-200/50 bg-white shadow-sm ${statusDetails.text}`}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-zinc-900">Active Incidents</h3>
                  <p className="text-xs text-zinc-500">Ongoing service issues and maintenance work</p>
                </div>
                <Link href="/status/history" className="text-xs font-bold text-zinc-600 hover:text-zinc-900 hover:underline">
                  Incident History →
                </Link>
              </div>
              ...
              {incidents.length === 0 ? (
                ...
                <div className="grid gap-4">
                  {incidents.map((incident) => (
                    <div key={incident.id} className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm space-y-4">
                      <div className="flex items-start justify-between border-b border-zinc-100 pb-3">
                        <div className="space-y-1">
                          <h4 className="text-sm font-bold text-zinc-950">{incident.title}</h4>
                          <span className="text-[10px] uppercase font-bold text-zinc-400">
                            Severity: {incident.severity}
                          </span>
                        </div>
                        <span className="rounded-full bg-zinc-100 px-2.5 py-1 text-[10px] font-bold text-zinc-600 border border-zinc-200 capitalize">
                          {incident.status}
                        </span>
                      </div>
                      ...
                      <div className="relative border-l border-zinc-200 pl-4 ml-1.5 space-y-4">
                        {[...incident.updates].reverse().map((update) => (
                          <div key={update.id} className="relative space-y-1">
                            ...
                            <p className="text-xs text-zinc-700 leading-relaxed">{update.message}</p>
