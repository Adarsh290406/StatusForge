...
export default function IncidentDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f9fafb] p-6 flex items-center justify-center">
        <div className="text-gray-400 animate-pulse font-semibold">Loading incident details...</div>
      </div>
    );
  }
  ...
  if (error || !incident) {
    return (
      <div className="min-h-screen bg-[#f9fafb] p-6 flex flex-col items-center justify-center space-y-4">
        <div className="text-red-600 font-semibold">{error || "Incident not found."}</div>
        <Link
        ...
        <div className="grid gap-6 md:grid-cols-3 items-start">
          <div className="md:col-span-2 space-y-6">
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
              <div className="flex items-start justify-between gap-4 flex-wrap pb-4 border-b border-gray-100">
                <div className="space-y-1">
                  <h1 className="text-xl font-bold text-gray-900 leading-snug">{incident.title}</h1>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-full bg-gray-50 border border-gray-200 px-2.5 py-0.5 text-[10px] font-bold text-gray-500 capitalize">
                      {incident.status}
                    </span>
                  </div>
                </div>
              ...
              {resolvedUpdate && (
                <div className="p-4 rounded-xl bg-green-50 border border-green-150 text-green-800 text-xs font-semibold leading-relaxed shadow-sm">
                  ✓ Resolved on {new Date(resolvedUpdate.createdAt).toLocaleString()}
                </div>
              )}

              <div className="text-sm text-gray-600">
                <span className="font-semibold text-gray-900">Affected Components:</span>{" "}
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-6">
              <h3 className="text-lg font-bold text-gray-900 border-b border-gray-100 pb-2">Timeline</h3>
              {incident.updates.length === 0 ? (
                <p className="text-xs text-gray-500 text-center py-6">No updates logged yet.</p>
              ) : (
                <div className="relative border-l border-gray-200 pl-6 ml-2 space-y-6">
                  {[...incident.updates].reverse().map((update) => (
                    <div key={update.id} className="relative">
                      {/* Timeline dot */}
                      <span className="absolute -left-8 top-1.5 h-3 w-3 rounded-full border border-gray-300 bg-white shadow-sm" />
                      <div className="space-y-1">
                        <p className="text-sm text-gray-700 leading-relaxed font-medium">{update.message}</p>
              ...
              <form onSubmit={handlePostUpdate} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full rounded border border-gray-200 px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 bg-white text-gray-900"
                  >
